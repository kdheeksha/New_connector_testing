...
|---|---|---|---|---|
| `triple_whale_meta_cac_first_touch_7d` | tsm_meta_pixel | `First Click` | `lifetime` | `nccpa` |
...
| `triple_whale_meta_ncp_firt_click` *(typo in source)* | tsm_meta_pixel | `First Click` | `lifetime` | `total_ncp` |
...
> uses `attribution_window = 'lifetime'` for First Click on Meta, same pattern
> as Google First Click. The `7d` refers to the reporting period, not the window.
...
| Metric group | Status |
|---|---|
| Meta First Click (lifetime window) | ✅ Validated (avg gap 0.44%, max 0.61%) |
| Meta Last Click (7d) | ✅ Validated (avg gap 0.84%, max 1.23%) |
